...
class OrderService
{
    ...
    public function __construct(Order $model, User $userModel, ProductOrder $productOrderModel, VariationOption $variationOptionModel, CouponUser $couponUserModel, AddressDetail $addressModel, Product $productModel)
    {
        $this->model = $model;
        ...
        return DB::transaction(function() use ($data){
            ...
            if($save->save()){
             foreach($data['products_orders'] as $item){
                 ...
                 if($item['variable_id'] != null){ // means its a variable product
                     $variation = $this->variationOptionModel::where('id', $item['variable_id'])->first();
                     $product_id = $variation->product_id;
                     updateProductUnit($variation->id, $this->variationOptionModel, $item['quantity']);
                 }else{
                     updateProductUnit($item['product_id'], $this->productModel, $item['quantity']);
                 }
