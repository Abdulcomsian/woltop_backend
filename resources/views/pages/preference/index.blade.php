<x-default-layout>
    @section('page-title', 'Manage Delivery Preferences')
    <div class="card">
        <!--begin::Card header-->
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-toolbar">
                <!--begin::Add user-->
                <a href="{{route('preference.create')}}" class="btn btn-primary">
    ...
    @include('partials.modals.preferences.edit')
    @include('partials.modals.preferences.delete')
    @push('scripts')
    {{ $dataTable->scripts() }}
    <script>
        ...
        function editItem(id){
            let url = `{{ route('preference.edit', ':id') }}`.replace(':id', id);
            fetch(url)
                .then((response) => {
                    if (!response.ok) {
                ...
                .then((res) => {
                    ...
                    document.querySelector("input[name='time']").value = res.data.time;
